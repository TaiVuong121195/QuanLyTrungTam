...
//serviceThongTin.ThongTinKhoaHoc();
//serviceThongTin.ThongTinNguoiDung();
...
function LuuHoTen() {
    ...
    document.getElementById("TenChuTK").innerHTML = `<i class="fa fa-user"></i> Xin chào ${hoten}`;
}
    ...
    $.ajax({
        ...
        success: function (ketqua) {
            ...
            for (var i = 0; i < ketqua.length; i++) {
                var urlApi = "http://sv.myclass.vn/api/QuanLyTrungTam/ChiTietKhoaHoc/" + ketqua[i].MaKhoaHoc;
                $.ajax({
                    type: 'GET',
                    url: urlApi,
                    success: function (res) {
                        ...
                        img.setAttribute("src", `resources/img/img${rand}.jpg`);
                        ...
                        theA_1.setAttribute("class", "hidden-sm chitiet btn btn-outline-primary btn-block");
...
function DanhSachCacKhoaHocHienCo() {
    ...
    $.ajax({
        ...
        success: function (ketqua) {
            ...
            for (var i = 0; i < ketqua.length; i++) {
                var cardKhoaHoc = document.createElement("div");
                cardKhoaHoc.setAttribute("class", "col-sm-12 col-md-6 col-lg-3 cardkhoahoc");
                ...
                theA_1.setAttribute("class", "hidden-sm btn btn-warning");
                ...
                var theA_2 = document.createElement("a");
                theA_2.setAttribute("class", "hidden-sm chitiet btn btn-outline-danger btn-block");
                theA_2.innerHTML = "Chi tiết";
                theA_2.setAttribute("href", "coursedetail.html");
...
function ThongTinTaiKhoan() {
    ...
    document.getElementById("MaLoaiNguoiDung").innerHTML = thongtinTK[0].MaLoaiNguoiDung;
    document.getElementById("TenLoaiNguoiDung").innerHTML = thongtinTK[0].TenLoaiNguoiDung;
}
        ...
        for(var i = 0; i < data.length; i++){
            ...
            if(name){
                if(name.toUpperCase().search(filter) != -1){
                    $('#result').append('<a href = "coursedetail.html" class="list-group-item list-group-item-action chitiet" makhoahoc = "'+data[i].MaKhoaHoc +'"><img src="'+`resources/img/img${rand}.jpg`+'" height="40" width="40" class="img-thumbnail" /> '+data[i].TenKhoaHoc);
                }
            }
